...
FROM customer AS C INNER JOIN `order` AS O ON C.CUS_ID = O.CUS_ID AND O.ORD_AMOUNT >=3000 
GROUP BY C.CUS_GENDER;	

-- 4)	Display all the orders along with product name ordered by a customer having Customer_Id=2
SELECT * FROM (select product.PRO_NAME, product.PRO_ID, supplier_pricing.PRICING_ID 
FROM supplier_pricing INNER JOIN product ON supplier_pricing.PRO_ID = product.PRO_ID ) AS supp_pro
...
SELECT C.CAT_NAME, sup_pro.minSP, sup_pro.PRO_ID, sup_pro.PRO_NAME, sup_pro.CAT_ID FROM (SELECT MIN(SP.SUPP_PRICE) AS minSP, P.PRO_ID, P.PRO_NAME, P.CAT_ID FROM supplier_pricing AS SP INNER JOIN product AS P ON SP.PRO_ID = P.PRO_ID GROUP BY P.CAT_ID) AS sup_pro
